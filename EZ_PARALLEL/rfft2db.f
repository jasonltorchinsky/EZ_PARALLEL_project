!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! RFFT2DB. Returns the inverse Fourier Transform (normalized) of a 2D DOUBLE
! PRECISION matrix.
!
...
! - matrix: The 2D DOUBLE PRECISION array which will be Fourier transformed
! (DOUBLE PRECISION, DIMENSION(dim1_len, dim2_len)).
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE RFFT2DB_EZP(dim1_len, dim2_len, overlap, matrix)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! - matrix: The 2D DOUBLE PRECISION array which will be Fourier transformed
! (DOUBLE PRECISION, DIMENSION(dim1_len, dim2_len)).
! - norm: The normalization coefficient for the transform (DOUBLE PRECISION).
...
! - matrix_tr: The 2D DOUBLE PRECISION array which will store the transpose of
! matrix during the Fourier transform (DOUBLE PRECISION,
...
DOUBLE PRECISION, DIMENSION(dim1_len, dim2_len) :: matrix
DOUBLE PRECISION, DIMENSION(dim2_len, dim1_len) :: matrix_tr
...
WSAVE1 = 0.0
CALL DFFTI(dim1_len, WSAVE1)

! Perform 1D FFFT on the dim1_len of matrix.
DO ii = 1, dim2_len
  CALL DFFTB(dim1_len, matrix(:,ii), WSAVE1) ! SW FFFT routine.
END DO
matrix_tr = TRANSPOSE(matrix)

WSAVE2 = 0.0
CALL DFFTI(dim2_len, WSAVE2)
! SW 1D (FFFT) initialization, fills WSAVE2.

! Perform 1D FFFT on the dim1_len of matrix_transpose.
DO ii = 1, dim1_len
  CALL DFFTB(dim2_len, matrix_tr(:,ii), WSAVE2)
END DO
matrix = TRANSPOSE(matrix_tr)

! Normalize the matrix in the inverse transform.
norm = 1.0/DBLE(dim1_len * dim2_len)
matrix = norm * matrix
...
! neighboring subgrids (DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE).
! - ii: Counting index used in DO loops (INTEGER).
...
! dim2_slice to be filled in by dim1_slice (INTEGER).
! - dim2_slice_tr: The 2D DOUBLE PRECISION array which will store the transpose
! of dim2_slice during the Fourier transform (DOUBLE PRECISION, DIMENSION(:,:),
! ALLOCATABLE).
! - WSAVE2: Work array to store prime factorization of dim2_len and tabulate
! trig functions for usage in SW's FFT routines (DOUBLE PRECISION,
...
DO ii = 1, dim2_len_dim1_slice
  CALL DFFTB(dim1_len, dim1_slice(:,ii), WSAVE1) ! SW FFFT routine.
END DO

! ~~~ Up to this point, we have FFT'd the grid along dim1.
...
! Calculate dim2_len for all dim2_slice's by adding up dim2_len for each
! processor's dim1_slice.
...
DO ii = 1, proc_count
  ...
  ! If dim2_slice_fill_in_param > proc_id, SEND then RECV.
  ...
  ! If dim2_slice_fill_in_param = proc_id, then fill in dim2_slice from your
  ! dim1_slice.
  ELSE
    dim2_slice(:,dim2_low_index_dim2_slice:dim2_high_index_dim2_slice) = &
    & dim1_slice(dim2_low_index_dim1_slice:dim2_high_index_dim1_slice,:)
...
! and we are ready to FFT along dim2_len.
ALLOCATE( &
...
! Perfom 1D FFT along dim1 of dim2_slice_tr.
  ! SW real forward 1D FFT (FFFT) initialization, fills WSAVE2.
WSAVE2 = 0.
CALL DFFTI(dim2_len_dim2_slice, WSAVE2)

DO ii = 1, dim1_len_dim2_slice_list(proc_id + 1)
  CALL DFFTB(dim2_len_dim2_slice, dim2_slice_tr(:,ii), WSAVE2)
END DO

dim2_slice = TRANSPOSE(dim2_slice_tr)

! ~~~ Up to this point, we have FFT'd along dim2_len, and now we must
! reconstruct the dim1_slice's.
...
! Normalize the dim1_slice for the inverse transform.
norm = 1.0/DBLE(dim1_len * dim2_len_total)
dim1_slice = norm * dim1_slice
...
CALL SHARE_SUBGRID_BOUNDARIES_DBLE_REAL_EZP(dim1_len, dim2_len, overlap, matrix)
