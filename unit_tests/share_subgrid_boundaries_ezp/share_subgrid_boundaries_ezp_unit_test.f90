...
!> Jason Turner
! DESCRIPTION:
!> \brief The unit test for the SHARE_SUBGRID_BOUNDARIES_EZP subroutine.
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

PROGRAM SHARE_SUBGRID_BOUNDARIES_UNIT_TEST
  ...
  CALL SHARE_SUBGRID_BOUNDARIES_TEST
  CALL CPU_TIME(end_time)

  PRINT *, "Execution time: ", end_time - start_time, "."
  PRINT *, "SHARE_SUBGRID_BOUNDARIES_EZP unit test complete. ", &
       "Normal termination."
...
CONTAINS
  ...
  SUBROUTINE SHARE_SUBGRID_BOUNDARIES_TEST
    ...
    ! Initialize each sub-grid.
    ALLOCATE(grid_dble(row_count_dble,col_count_dble))
    DO j = 1, col_count_dble
       DO i = 1, row_count_dble
          grid_dble(i,j) = i + col_spc_dble*j + proc_id
       END DO
    END DO

    ALLOCATE(grid_dcmplx(row_count_dcmplx,col_count_dcmplx))
    DO j = 1, col_count_dcmplx
       DO i = 1, row_count_dcmplx
          grid_dcmplx(i,j) = i + col_spc_dcmplx*j + (0.0,1.0)*proc_id
       END DO
    END DO

    PRINT *, " "
    PRINT *, "proc_id: ", proc_id, " grid_dble: ", grid_dble
    PRINT *, "proc_id: ", proc_id, " grid_dcmplx: ", grid_dcmplx

    ! Communicate sub-grid boundaries.
    CALL SHARE_SUBGRID_BOUNDARIES_EZP(decomp_id_dble, grid_dble)
    CALL SHARE_SUBGRID_BOUNDARIES_EZP(decomp_id_dcmplx, grid_dcmplx)
    ...
    DEALLOCATE(grid_dble)
    DEALLOCATE(grid_dcmplx)

    PRINT *, "MPI_DOUBLE_COMPLEX: ", MPI_DOUBLE_COMPLEX
    PRINT *, "MPI_DOUBLE_PRECISION: ", MPI_DOUBLE_PRECISION
    
    CALL FIN_EZP

    RETURN

  END SUBROUTINE SHARE_SUBGRID_BOUNDARIES_TEST

END PROGRAM SHARE_SUBGRID_BOUNDARIES_UNIT_TEST
