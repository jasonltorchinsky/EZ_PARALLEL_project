function [full_grid] = MERGE_SUBGRIDS(proc_count, overlap, file_count, output_freq, layer)
...
for file_num = 0:file_count-1
   ...
   file_name = sprintf('layer%d_%08d_%04d.csv', layer, output_freq*file_num, 0);
   subgrid = readmatrix(file_name);
   full_grid = subgrid(1:end,:);
   % Add all other sub-grids to full grid.
   for proc_id = 1:proc_count-1
       file_name = sprintf('layer%d_%08d_%04d.csv', layer, output_freq*file_num, proc_id);
       subgrid = readmatrix(file_name);
       full_grid = [full_grid(1:end-overlap,:); subgrid(1+overlap:end,:)];
   end
   full_grid = full_grid(:,1:end-1);
   out_file_name = sprintf('layer%d_%08d.csv', layer, output_freq*file_num);
