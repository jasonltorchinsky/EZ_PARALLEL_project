...
COMPLEX(dp), DIMENSION(x_len, y_len, 2):: frequency_pot_vorticity_grid
...
CALL CFFT2DF(x_len, y_len, 0, frequency_pot_vorticity_grid(:,:,1))
CALL CFFT2DF(x_len, y_len, 0, frequency_pot_vorticity_grid(:,:,2))
!~~

! Set up hyperviscosity for time-stepping.
! Store 1D wavenumbers to make hypervisocsity operator.
ALLOCATE(wavenumbers_1d(x_len))
wavenumbers_1d(:) = 0.0_dp
wavenumbers_1d(x_len) = -1.0_dp
DO i = 1, x_len/2
  wavenumbers_1d(i + 1) = wavenumbers_1d(i) + 1.
  wavenumbers_1d(x_len - i) = wavenumbers_1d(x_len - i + 1) - 1.
END DO
! Create the spectral biharmonic operator for hypervisocsity.
ALLOCATE(spectral_biharm(x_len, y_len, 2))
...
REAL(dp), DIMENSION(x_len, y_len, 2) :: spectral_biharm
...
CALL JACOBIAN_EKMAN_SHEAR(freq_pot_vort_grid, y_len, dt, &
...
!PRINT *, "proc_id: ", proc_id, " frequency_pot_vorticity_1: ", frequency_pot_vorticity_1(:,:,1)
! num_proc = 1 and num_proc = 2 differ here, figure out why.
...
CALL JACOBIAN_EKMAN_SHEAR(frequency_pot_vorticity_1, y_len, dt, &
deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
CALL JACOBIAN_EKMAN_SHEAR(frequency_pot_vorticity_2, y_len, dt, &
deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
CALL JACOBIAN_EKMAN_SHEAR(frequency_pot_vorticity_3, y_len, dt, &
deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
CALL JACOBIAN_EKMAN_SHEAR(frequency_pot_vorticity_4, y_len, dt, &
deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
CALL JACOBIAN_EKMAN_SHEAR(frequency_pot_vorticity_5, y_len, dt, &
deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
! error_control DOES NOT MATCH UP FOR 1 AND 2 PROCESSORS

!~~ CHANGED FOR PARALLEL
CALL CFFT2DB(x_len, y_len, 0, error_control(:,:,1))
CALL CFFT2DB(x_len, y_len, 0, error_control(:,:,2))
!~~

! ADDED TO PARALLEL
CALL MAXVAL_MPI(x_len*y_len*2, ABS(error_control), error_toler_1)
PRINT *, "proc_id: ", proc_id, " MAXVAL(ABS(error_control)): ", error_toler_1
!~~ CHANGED FOR PARALLEL
...
IF (error_toler_1 .GT. error_toler) THEN
  dt = 0.75_dp * dt
  PRINT *, "proc_id: ", proc_id, " Step error too large, retrying..."
  GO TO 101
END IF
!~~
...
! CHANGED FOR PARALLEL
PRINT *, "proc_id: ", proc_id, " Time at current step: ", time, &
" dt at current step: ", dt, " Error at current step: ", error_toler_1, "."
!~~
...
!~~ CHANGED FOR PARALLEL
CALL CFFT2DB(x_len, y_len, 0, physical_pot_vorticity_grid(:,:,1))
CALL CFFT2DB(x_len, y_len, 0, physical_pot_vorticity_grid(:,:,2))
!~~
...
!~~ CHANGED FOR PARALLEL
CALL CFFT2DF(x_len, y_len, 0, freq_pot_vort_grid(:,:,1))
CALL CFFT2DF(x_len, y_len, 0, freq_pot_vort_grid(:,:,2))
!~~
...
!~~ CHANGED FOR PARALLEL
PRINT *, "proc_id: ", proc_id, " dt for next step: ", dt, "."
!~~

!~~ CHANGED FOR PARALLEL.
CALL MAXVAL_MPI(x_len*y_len*2, REAL(ABS(physical_pot_vorticity_grid), dp), &
max_pot_vorticity)

IF ((max_pot_vorticity .GT. pot_vorticity_bound) .AND. (proc_id .EQ. 0)) THEN
