...
CONTAINS
  ...
  SUBROUTINE TIME_STEP_SCHEME(stepParity)
    ...
    REAL(dp) :: whiteNoise1(1:numPts-2,1:numPtsY-2), whiteNoise2(1:numPts-2,1:numPtsY-2)
    !< White noise for the stochasticity of the time-step. CHANGED FOR PARALLEL
    ...
    colWtrVpr(1:numPts-2, 1:numPtsY-2, stepParity) = &
         colWtrVpr(1:numPts-2, 1:numPtsY-2, prevStepParity) &
         + dt * deterForce &
         + dt * interCoeff * (colWtrVpr(0:numPts-3, 1:numPtsY-2, prevStepParity) &
         + colWtrVpr(2:numPts-1, 1:numPtsY-2, prevStepParity) &
         + colWtrVpr(1:numPts-2, 0:numPtsY-3, prevStepParity) &
         + colWtrVpr(1:numPts-2, 2:numPtsY-1, prevStepParity) &
         - 4.0_dp * colWtrVpr(1:numPts-2, 1:numPtsY-2, prevStepParity)) &
         - (dt / dampCoeff) * (colWtrVpr(1:numPts-2, 1:numPtsY-2, prevStepParity) &
         - relaxTrgt) + whiteNoise1

    CALL SHARE_SUBGRID_BDRY(colWtrVpr(:,:,stepParity), sch) !< ADDED TO PARALLEL.
