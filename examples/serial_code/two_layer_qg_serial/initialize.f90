...
INTEGER(qb) :: xy_len, num_timesteps, biharm_order, output_freq
...
PUBLIC :: xy_len, num_timesteps, biharm_order, output_freq
...
NAMELIST /model/ xy_len, dx, dy, num_timesteps, init_dt, pot_vorticity_bound, &
& deform_wavenumber, rotat_wavenumber, vertical_shear, ekman_friction_coeff, &
...
! layer 2 (:, :, 2).
ALLOCATE(physical_pot_vorticity_grid(xy_len, xy_len, 2))
