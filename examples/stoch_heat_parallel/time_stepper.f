...
!/* ADDED TO PARALLEL
IF ((proc_id .EQ. 0) .AND. (dt .GT. stability_constraint)) THEN
  PRINT *, 'WARNING: Time step size (dt) exceeds the CFL parameter. ', &
  & 'Simulation may be unstable.'
END IF
!*/
...
CALL SHARE_SUBGRID_BOUNDARIES_DBLE_REAL_EZP(x_len, y_len, 1_qb, &
  & temp_grid(:,:,step_parity)) ! ADDED TO PARALLEL
